...
#define DRIVER_VERSION "1.3.2.0-esxi1.0"
...
#ifdef __VMKLNX__
u32 aqc111_get_rx_csum(struct net_device *net)
{
	struct usbnet *dev = netdev_priv(net);
	struct aqc111_data *aqc111_data = (struct aqc111_data *)dev->data[0];

	return aqc111_data->rx_checksum;
}

int aqc111_set_rx_csum(struct net_device *net, u32 rx_csum)
{
	struct usbnet *dev = netdev_priv(net);
	struct aqc111_data *aqc111_data = (struct aqc111_data *)dev->data[0];

	aqc111_data->rx_checksum = rx_csum;
...
#endif
#ifdef __VMKLNX__
	.get_rx_csum = aqc111_get_rx_csum,
	.set_rx_csum = aqc111_set_rx_csum,
...
#ifdef __VMKLNX__
static void aqc111_vlan_rx_kill_vid(struct net_device *net, u16 vid)
#else
...
#ifdef __VMKLNX__
static void aqc111_vlan_rx_add_vid(struct net_device *net, u16 vid)
#else
...
static int aqc111_stop_net(struct net_device *net)
{
	struct usbnet *dev = netdev_priv(net);
	struct aqc111_data *aqc111_data = (struct aqc111_data *)dev->data[0];
	int err = 0;
	...
	if (aqc111_data->usbnet_stop)
		err = aqc111_data->usbnet_stop(net);
...
static int aqc111_bind(struct usbnet *dev, struct usb_interface *intf)
{
	 ...
	 * Already set by usbnet_probe
	 */

	dev->net->change_mtu = aqc111_change_mtu,
	dev->net->set_mac_address = aqc111_set_mac_addr,
	dev->net->vlan_rx_add_vid = aqc111_vlan_rx_add_vid,
	dev->net->vlan_rx_kill_vid = aqc111_vlan_rx_kill_vid,
	/*Ahahaha dirty trick*/
	aqc111_data->usbnet_stop = dev->net->stop;
	dev->net->stop = aqc111_stop_net;
#else
...
#else
		new_skb = alloc_skb(pkt_len + NET_IP_ALIGN, GFP_ATOMIC);
#endif
		if (!new_skb)
			goto err;
...
#ifdef __VMKLNX__
static int __init aqc111_init(void)
{
	return usb_register(&aq_driver);
}
module_init(aqc111_init);

static void __exit aqc111_exit(void)
{
	usb_deregister(&aq_driver);
}
module_exit(aqc111_exit);
#else
